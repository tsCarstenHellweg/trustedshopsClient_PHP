...
var _tsid = 'XXXXXXXXXXXXXXXXXXXXXXXXXXXXX'; 
...
var getTSstars = function(zahl, htmlText){
    ...
    $('.ts-stars-inner').width(starPercentageRounded);
    $('.tsNote').html(zahl);
    $('.tsNoteDescription').html(description); 
...
if(sessionStorage.getItem(ts_item)){
    ...
    { 
        var content = '<div id="noteDescription">' + ' ' + description 
                    + ' ' + '</div><div>' + '<span itemprop="ratingValue">' 
                    + result_ts + '</span> /' + '<span itemprop="bestRating">' 
                    + 5 + '</span> of '  + '<span itemprop="ratingCount"> ('
                    + count + ')   </span> </div>'; 
    }    
